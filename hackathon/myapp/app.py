...
from forms import KeyExchangeForm
from wtforms.validators import DataRequired, Email, ValidationError
from wtforms import StringField, SubmitField
from wtforms.fields import DateField  # Add this import
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:@localhost/otp'
...
class GeneratedKey(db.Model):
    ...
    generated_key = db.Column(db.String(10), nullable=False)
...
def verify_key():
    ...
    if entered_key == expected_key:
        ...
        # Create and store the generated key
        generated_key = GeneratedKey(username=username, generated_key=entered_key)
        db.session.add(generated_key)
        db.session.commit()
...
class EditProfileForm(FlaskForm):
    ...
    gender = StringField('Gender', validators=[DataRequired()])
    address = StringField('Address')
    date_of_birth = DateField('Date of Birth', format='%d.%m.%Y')  # Add this line for the date field
...
def edit_profile():
    ...
    #     db.session.commit()
    #     flash('Profile updated successfully!', 'success')
    #     return redirect(url_for('profile1'))  # Corrected to 'profile' endpoint
