...
def login():
    ...
    if form.validate_on_submit():
        ...
        # Store the verification key and username in the session
...
def verify():
    ...
    if request.method == 'POST':
        if 'username' in session:
            ...
            expected_key_record = GeneratedKey.query.filter_by(username=user.username).order_by(GeneratedKey.id.desc()).first()

            if expected_key_record:
                expected_key = expected_key_record.generated_key
                ...
                # Handle the case where there's no expected key record
                flash('Verification key record not found. Please log in again.')
                return redirect(url_for('login'))
        else:
